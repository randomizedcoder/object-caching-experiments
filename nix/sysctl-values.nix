...
#
# Extracted to its own data file so BOTH consumers stay in sync:
#   - NixOS:  modules/sysctls.nix  → boot.kernel.sysctl = import …
#   - Ubuntu: ubuntu-client.nix    → rendered to /etc/sysctl.d/ (system-manager
#             has no boot.kernel.sysctl; `boot` is a raw stub it ignores).
{
